TODO:
    - Redirects (return directive): Implemented, working.
    - Missing Content-Length on all responses
        Missing Content-Length on all responses: Some paths (e.g., POST 201) should include Content-Length: 0 (appears to be done now).
    - Magic numbers: TIMEOUT_LIMIT = 5, MAX_EVENTS = 64, buffer size 8192 — no explanation.
    - Some silent failures: If epoll operations fail, only std::cerr is written; program doesn't exit.
    - If epoll operations fail, only std::cerr is written; program doesn't exit.
